...
def delete_part(part_id: str) -> tuple[bool, str]:
    with get_conn() as conn:
        used = conn.execute(
            "SELECT COUNT(*) FROM bom WHERE child_id = ?", (part_id,)
        ).fetchone()[0]
        if used:
            return False, f"Cannot delete: part is a child in {used} BOM relationship(s)."
        has_children = conn.execute(
            "SELECT COUNT(*) FROM bom WHERE parent_id = ?", (part_id,)
        ).fetchone()[0]
        if has_children:
            return False, f"Cannot delete: this assembly has {has_children} BOM component(s). Remove all BOM children first."
        conn.execute("DELETE FROM bom WHERE parent_id = ?", (part_id,))
        conn.execute("DELETE FROM project_items WHERE part_id = ?", (part_id,))
    ...
    try:
        with get_conn() as conn:
            ...
            if field in ('pkg_size', 'pkg_cost'):
                row = conn.execute(
                    "SELECT type,pkg_size,pkg_cost FROM parts WHERE part_id=?", (part_id,)
                ).fetchone()
                if row and row['type'] in PART_TYPES_STATIC_COST:
                    ps, pc = float(row['pkg_size'] or 1), float(row['pkg_cost'] or 0)
                    if ps > 0:
                        conn.execute("UPDATE parts SET unit_cost=? WHERE part_id=?",
                                     (round(pc/ps, 6), part_id))
        return True, "Saved."
    except Exception as e:
        return False, str(e)
    ...
    with get_conn() as conn:
        rows = conn.execute("""
            SELECT b.child_id, b.qty, b.sort_order,
                   p.plain_desc, p.type, p.unit_cost, p.uom, p.labor_hrs, p.status
...
def build_bom_tree(parent_id: str, depth: int = 0, visited: set = None, _conn = None) -> dict:
    if _conn is None:
        with get_conn() as conn:
            return build_bom_tree(parent_id, depth, visited, conn)
    ...
    part = _conn.execute("SELECT * FROM parts WHERE part_id=?", (parent_id,)).fetchone()
    if not part:
        return {'part_id': parent_id, 'error': 'Not found', 'children': []}
    children_rows = _conn.execute(
        "SELECT child_id, qty FROM bom WHERE parent_id=? ORDER BY sort_order, child_id",
        (parent_id,)
    ).fetchall()
    ...
    for row in children_rows:
        child_tree = build_bom_tree(row['child_id'], depth + 1, visited, _conn)
...
def run_rollup(part_id: str, cycle_visited: set = None, memo: dict = None, _conn = None) -> tuple[float, float]:
    """
    Calculates and stores pure material cost and labor hours for FAB/ASSY parts.
    Returns (material_cost, labor_hrs).
    """
    if _conn is None:
        with get_conn() as conn:
            return run_rollup(part_id, cycle_visited, memo, _conn=conn)
    ...
    part = _conn.execute(
        "SELECT type, unit_cost, labor_hrs FROM parts WHERE part_id=?", (part_id,)
    ).fetchone()
    ...
    children = _conn.execute(
        "SELECT child_id, qty FROM bom WHERE parent_id=?", (part_id,)
    ).fetchall()
    ...
    for child in children:
        ...
        c_mat, c_lbr = run_rollup(child['child_id'], cycle_visited, memo, _conn=_conn)
    ...
    _conn.execute("UPDATE parts SET unit_cost=?, rolled_labor_hrs=? WHERE part_id=?",
                 (round(total_mat, 6), round(total_lbr, 4), part_id))
...
def get_where_used(part_id: str) -> list[dict]:
    """
    Return every direct parent with its qty, plus the full ancestor chain
    to the top-level assembly. Each entry:
      { parent_id, parent_desc, parent_type, qty, path: [top→...→parent] }
    """
    results = []
    with get_conn() as conn:
        _collect_where_used(conn, part_id, [], results, set())
    return results

def _collect_where_used(conn, part_id: str, current_path: list, results: list, visited: set):
    ...
    """, (part_id,)).fetchall()
    for par in parents:
        path = [par['parent_id']] + current_path
        results.append({
            'parent_id':   par['parent_id'],
            'parent_desc': par['plain_desc'],
            'parent_type': par['type'],
            'qty':         par['qty'],
            'path':        path,   # top-level first
        })
        _collect_where_used(conn, par['parent_id'], path, results, visited)
...
def explode_bom_flat(parent_id: str, qty_mult: float = 1.0,
                     visited: set = None, _conn = None) -> list[dict]:
    """
    Fully exploded flat BOM: every leaf/node with rolled quantities.
    Returns list of {part_id, plain_desc, type, unit_cost, uom,
                     total_qty, depth, optional, path_str}
    """
    if _conn is None:
        with get_conn() as conn:
            return explode_bom_flat(parent_id, qty_mult, visited, conn)
    ...
    children = _conn.execute(
        "SELECT b.child_id, b.qty FROM bom b WHERE b.parent_id=? ORDER BY b.sort_order, b.child_id",
        (parent_id,)
    ).fetchall()

    rows = []
    for child in children:
        p = _get_part(_conn, child['child_id'])
        if not p: continue
        qty = child['qty']
        optional = (qty == 0)
        ...
        rows.append({
            'part_id':    child['child_id'],
            'plain_desc': p['plain_desc'],
            'type':       p['type'],
        ...
        if p['type'] in PART_TYPES_WITH_BOM:
            sub = explode_bom_flat(child['child_id'], eff_qty, visited, _conn)
            rows.extend(sub)
...
TYPES      = ["PRT", "RAW", "FAB", "ASSY"]
CATEGORIES = ["DLT", "SBX", "GRV", "CAL", "BLC", "LAB", "STD"]
PART_TYPES_WITH_BOM    = {"FAB", "ASSY"}
PART_TYPES_STATIC_COST = {"PRT", "RAW"}
PART_STATUSES = ["ACTIVE", "OBSOLETE"]
...
def generate_pick_list(project_id: str) -> list[dict]:
    ...
    with get_conn() as conn:
        items = conn.execute(
            "SELECT pi.part_id, pi.qty, pi.item_type, p.plain_desc, p.type, p.unit_cost, p.uom "
    ...
    with get_conn() as conn:
        return [dict(r) for r in conn.execute("""
            SELECT pi.id, pi.part_id, pi.qty, pi.picked,
                   ...
                   p.plain_desc, p.type, p.unit_cost, p.uom
            FROM project_items pi JOIN parts p ON p.part_id=pi.part_id
            WHERE pi.project_id=?
            ORDER BY
                CASE pi.item_type
